	...
	def actuate(self, action: str, driveTime: float = SHUTTER_ACTUATION_TIME):
		...
		with self._lock:
			if action == 'open':
				...
				print("Shutter command not recognized!")
...
class Luminometer():
	...
	def _btn1_callback(self, channel):
		...
		print(f"Button held for {duration} seconds ")
	...
	def measure(self, measure_time:int = 30, shutter_time: int = 2, measurementType: str = 'measure')-> List[float]:
		"""
		Arguments:
		...
		with self._measureLock:
			...
			try:
				...
				while self._rsc < self.nRawSamples:
					...
					if (self._sc > sampleCount) and ((self._sc % 2)==0) and (self._sc > 2):
						...
						try:
							...
							print('Display queue full. Could not display result')
				...
				print(f'Exception occured during measurement: {exc}')
			finally:
				self.t1 = time.time()
				self._measuring = False
